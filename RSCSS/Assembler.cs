...
namespace RSCSS
{
    public class Assembler
    {
        ...
        public AssemblyResults Assemble(string source, int location, Box box, CPU CPUBox, Memory memory)
        {
                    ...
                    else if (AssemblyInstructions.isMnemonic(tokens[0 + offset]))
                    {
                        ...
                        if (AssemblyInstructions.ExpectsOperands(tokens[0 + offset]) == 1)
                        {
                            ...
                            if (tokens.Length - offset < 2)
                            {
                                errorCount++;
                                assemblyErrorList.Add(new AssemblyError(
                                   lineIndex, tokens[0 + offset].ToUpper() + " expects "
                                   + "one operand"));
                            }
                            else if (tokens.Length - offset > 2)
                            {
                                errorCount++;
                                assemblyErrorList.Add(new AssemblyError(
                                   lineIndex, tokens[0 + offset].ToUpper() + " does not "
